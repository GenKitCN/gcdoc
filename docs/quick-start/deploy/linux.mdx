...
Restart=on-failure
RestartSec=5s
WorkingDirectory=/path/to/grasscutter
ExecStart=/usr/bin/screen -dm -S grasscutter bash -c 'cd /path/to/grasscutter;java -jar ./grasscutter.jar;exec bash'
RemainAfterExit=yes
...
systemd部分由 [@qwerzl](https://github.com/qwerzl) 贡献
